...
public class MyClass implements MyObject {
    ...
    private ArrayList<String> associateName = new ArrayList<>();
    ...
    public void associate(MyClass c, UmlAssociation a, String endName) {
        //System.err.println(this.name + "associate" + c.getname());
        //this.ass.add(a);
        this.ass++;
        this.association.add(c);
        if (endName != null) {
            this.associateName.add(endName);
        }
    ...
    public Set<AttributeClassInformation> check002part1() {
        HashSet<AttributeClassInformation> res =
            new HashSet<AttributeClassInformation>();
        HashSet<String> res2 = new HashSet<>();
        for (String s : mapAttribute.keySet()) {
            if (mapAttribute.get(s).size() >= 2) {
                res2.add(s);
            }
        }
        ArrayList<String> tmp1 = new ArrayList<>();
        tmp1.addAll(associateName);
        tmp1.addAll(mapAttribute.keySet());
        HashSet<String> tmp2 = new HashSet<>(tmp1);
        for (String i : tmp2) {
            tmp1.remove(i);
        }
        res2.addAll(tmp1);
        for (String i : res2) {
            res.add(new AttributeClassInformation( i,this.name));
        }
        return res;
    }

    /*public boolean check002part2(String name) {
        return this.mapAttribute.containsKey(name);
    }*/

    /*public boolean check009() {
        HashSet<MyInterface> tmp = getAllInterface();
